...
const sendMessage = async(req, res) => {
    try {
        ...
        if(file){
            ...
            return response(res,400,"message content is rewuired")
        }
...
const getConversations = async(req, res) => {
    ...
    try {
        let conversations = await Conversation.find({
            participants: userId,
        }).populate("participants", "username profilePicture lastMessage isOnline")
        .populate({
            path: "lastMessage",
            populate: [
                {
...
//* get messages of spacific conversation */
...
const getMessages = async(req, res) => {
    ...
    try {
        ...
        if(!conversation.participants.includes(userId)){
            return response(res, 403, "Not authorized to view messages in this conversation");
        }
...
const deleteMessage = async(req, res) => {
    ...
    try {
        ...
        await Message.findByIdAndDelete(messageId);
         ...
         // emit socket event
        if (req.io && req.socketUserMap) {
           const reciverSocketId = req.socketUserMap.get(message.receiver._id.toString());
           if(reciverSocketId){
