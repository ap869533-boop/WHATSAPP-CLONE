...
const Layout = ({ children, isThemeDialogOpen, toggleThemeDialog, isStatusPreviewOpen, statusPreviewContent }) => {
    const selectedContact = userLayoutStore(state => state.selectedContect);
    const setSelectedContact = userLayoutStore(state => state.setSelectedContect);
    ...
    return (
        <div className={` min-h-screen flex relative ${theme === 'dark' ? 'bg-[#111b21] text-white' : "bg-gray-100 text-black"}`}>
            {isMobile && <Sidebar />}
                    ...
                    <div className={`${theme === 'dark' ? 'bg-[#202c33] text-white' : "bg-white text-black"} p-6 rounded-lg shadow-lg max-w-sm w-full `}>
                        ...
                        <button onClick={toggleThemeDialog}
                            className="bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 
                             px-4 rounded mt-6 w-full transition duration-200 ">
                            Close
                        </button>
                    </div>
                </div>
